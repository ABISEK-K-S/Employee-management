...
app.post("/login", function (req, res) {
  ...
  con.connect(function (err) {
    con.query(
      `Insert into employee (first_name,last_name,department,email,phone) VALUES
...
app.post("/update", function (req, res) {
  ...
  con.connect(function (err) {
    con.query(`update employee set first_name="${req.body.first_name}",last_name="${req.body.last_name}",department="${req.body.department}",
       email="${req.body.email}",phone="${req.body.phone}" where employee_id="${req.body.employee_id}"`),
      function (err, result) {
        if (err) throw err;
        console.log("inserted successfully");
        res.end("yes");
